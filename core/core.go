...
import (
	...
	"database/sql"
	"fmt"
	"github.com/waltcow/sql2pb/tools/stringx"
	"log"
	"regexp"
	"sort"
	"strings"
...
func (m Message) GenDefaultMessage(buf *bytes.Buffer) {
	...
	for _, field := range m.Fields {
		if isInSlice([]string{"version", "delete_state", "delete_time"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
func (m Message) GenRpcAddReqRespMessage(buf *bytes.Buffer) {
	...
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"id", "create_time", "update_time", "version", "delete_state", "delete_time"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
...
func (m Message) GenRpcUpdateReqMessage(buf *bytes.Buffer) {
	...
	var filedTag int
	for _, field := range m.Fields {
		if isInSlice([]string{"create_time", "update_time", "version", "delete_state", "delete_time"}, field.Name) {
			continue
		}
		filedTag++
		field.tag = filedTag
